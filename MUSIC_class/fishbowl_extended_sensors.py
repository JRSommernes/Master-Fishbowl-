...
class Fishbowl:
    def __init__(self,N_sensors,radius,wl,n,mur,epsr,k_0,dipoles,M_timepoints,voxel_rad):
    ...
    def angular_dyadic_green(self,theta):
        phi = self.sensor_phi
        x = self.radius*np.cos(phi)*np.sin(theta)
        y = self.radius*np.sin(phi)*np.sin(theta)
        z = self.radius*np.cos(theta)
        dipole = np.array([x,y,z])
        G = self.dyadic_green(dipole)

        return G.flatten()

    def extended_sensor_field(self,dipole):
        def int2(phi):
            self.sensor_phi = phi
            th = self.sensor_theta
            int = quad_vec(self.angular_dyadic_green,th,th+self.voxel_rad)[0]
            return int

        theta = np.arctan2(np.sqrt(dipole[0]**2+dipole[1]**2),dipole[2])
        phi = np.arctan2(dipole[1],dipole[0])
        rad = self.voxel_rad/self.radius

        self.sensor_theta = theta
        int = quad_vec(int2,phi,phi+self.voxel_rad)[0]
        int = int.reshape(self.N_sensors,3,3)
    ...
    def sensor_field(self,polarizations):
        ...
        for i in range(len(self.dipoles)):
            G = self.extended_sensor_field(self.dipoles[i]).transpose(1,0,2)
    ...
    def check_resolvability(self):
        ...
        for i in range(len(pos)):
            A[i] = self.extended_sensor_field(pos[i])
