...
def scatter_MUSIC(I,sensors,emitters,N_recon,FoV,k_0,E_scatter,dipoles):
    ...
    for I in (I_x,I_y,I_z):
        ...
        A_test = np.zeros((N_sensors,K,9),dtype=np.complex128)
        for k in range(K):
            tmp = dyadic_green(sensors,plane[k],k_0)
            A_test[:,k] = tmp.reshape(tmp.shape[0],-1)
        ...
        V_tilde = V[:,NN]
        P_prime = np.identity(len(V_tilde)) - V_tilde@V_tilde.T.conj()

        njy = np.zeros(K,dtype=np.complex128)
        for i in range(9):
            njy += np.sum(P_prime@A_test[:,:,i],axis=0)
